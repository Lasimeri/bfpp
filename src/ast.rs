...
//
// NOTE: String interning for SubDef/SubCall names was considered but skipped.
// Changing String → u32 (interned ID) would touch 6 files and 70+ call sites
// across parser, optimizer, analyzer, codegen, and lexer. The overhead of
// String clones is negligible compared to codegen and CC time.
...
pub enum AstNode {
    ...
    // ── Auto-parallelism (optimizer synthetic) ────────────────────
    // These are generated by pass_auto_parallel when it detects provably
    // independent loop iterations or subroutine calls.
    ParallelLoop {
        body: Vec<AstNode>,
        stride: usize,
        trip_count: Option<u64>,  // Some(N) for statically-known, None for runtime-detected
    },
    #[allow(dead_code)]
    ParallelCalls(Vec<String>),  // subroutine names that can run concurrently

    // ── GPU transpilation (optimizer synthetic) ─────────────────
    // Generated by pass_detect_gpu_loops when a data-parallel loop can be
    // offloaded to OpenCL. The kernel_source is the generated OpenCL C string.
    GpuLoop {
        trip_count: u64,
        stride: usize,
        body: Vec<AstNode>,
