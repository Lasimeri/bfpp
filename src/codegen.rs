...
fn generate_c(program: &Program, opts: &CodegenOptions, uses_ffi: bool, intrinsics: &IntrinsicUsage) -> (String, Option<SplitSources>) {
    ...
    let header = emit_header(opts, uses_ffi, intrinsics, do_split);
...
fn emit_header(opts: &CodegenOptions, uses_ffi: bool, intrinsics: &IntrinsicUsage, do_split: bool) -> String {
    ...
    if intrinsics.threading {
        ...
        // parallel compilation (split TUs) reference these symbols.
...
fn emit_intrinsic(out: &mut String, name: &str, ctx: &mut GenCtx) {
    match name {
        ...
        // ── String operations ────────────────────────────────────
        "__strcmp" => {
            // Input: tape[ptr]=addr_a, tape[ptr+4]=addr_b (both null-terminated on tape)
            // Output: tape[ptr] = 0 if equal, <0 if a<b, >0 if a>b
            // Uses ptr+4 stride for %4 cell mode compatibility.
            indent(out, ctx.indent);
            out.push_str("{ int _a = (int)bfpp_get(ptr) & TAPE_MASK; int _b = (int)bfpp_get(ptr+4) & TAPE_MASK; int _r = 0; while (tape[_a] == tape[_b] && tape[_a] != 0) { _a++; _b++; } _r = (int)tape[_a] - (int)tape[_b]; bfpp_set(ptr, (uint64_t)(int64_t)_r); }\n");
        }
        "__strlen" => {
        ...
        "__strcpy" => {
            // Input: tape[ptr]=dst_addr, tape[ptr+4]=src_addr
            // Output: copies null-terminated string from src to dst
            indent(out, ctx.indent);
            out.push_str("{ int _d = (int)bfpp_get(ptr) & TAPE_MASK; int _s = (int)bfpp_get(ptr+4) & TAPE_MASK; while (tape[_s]) { tape[_d++] = tape[_s++]; } tape[_d] = 0; }\n");
        }

        // ── Array operations ─────────────────────────────────────
        // All use ptr+4 stride for %4 cell compatibility.
        "__array_insert" => {
            // Input: tape[ptr]=array_addr, tape[ptr+4]=index, tape[ptr+8]=element_size,
            //        tape[ptr+12]=count, tape[ptr+16]=value_addr (source)
            indent(out, ctx.indent);
            out.push_str("{ int _base = (int)bfpp_get(ptr) & TAPE_MASK; int _idx = (int)bfpp_get(ptr+4); int _esz = (int)bfpp_get(ptr+8); int _cnt = (int)bfpp_get(ptr+12); int _vsrc = (int)bfpp_get(ptr+16) & TAPE_MASK; int _off = _base + _idx * _esz; int _tail = (_cnt - _idx) * _esz; if (_tail > 0) memmove(&tape[_off + _esz], &tape[_off], _tail); memcpy(&tape[_off], &tape[_vsrc], _esz); }\n");
        }
        "__array_remove" => {
            // Input: tape[ptr]=array_addr, tape[ptr+4]=index, tape[ptr+8]=element_size,
            //        tape[ptr+12]=count
            indent(out, ctx.indent);
            out.push_str("{ int _base = (int)bfpp_get(ptr) & TAPE_MASK; int _idx = (int)bfpp_get(ptr+4); int _esz = (int)bfpp_get(ptr+8); int _cnt = (int)bfpp_get(ptr+12); int _off = _base + _idx * _esz; int _tail = (_cnt - _idx - 1) * _esz; if (_tail > 0) memmove(&tape[_off], &tape[_off + _esz], _tail); }\n");
        }
        ...
        "__hashmap_init" => {
            // Input: tape[ptr]=map_addr, tape[ptr+4]=capacity
            indent(out, ctx.indent);
            out.push_str("{ int _addr = (int)bfpp_get(ptr) & TAPE_MASK; int _cap = (int)bfpp_get(ptr+4); bfpp_set(_addr, (uint64_t)_cap); bfpp_set(_addr + 4, 0); memset(&tape[_addr + 8], 0, _cap * 40); }\n");
        }
        "__hashmap_get" => {
            // Input: tape[ptr]=map_addr, tape[ptr+4]=key_addr
            // Output: tape[ptr]=value, tape[ptr+4]=found(0/1)
            indent(out, ctx.indent);
            out.push_str("{ int _maddr = (int)bfpp_get(ptr) & TAPE_MASK; int _kaddr = (int)bfpp_get(ptr+4) & TAPE_MASK; int _cap = (int)bfpp_get(_maddr); uint32_t _h = 5381; for (int _i = _kaddr; tape[_i]; _i++) _h = _h * 33 + tape[_i]; int _found = 0; for (int _p = 0; _p < _cap; _p++) { int _s = ((_h + _p) % _cap) * 40 + _maddr + 8; if (tape[_s] == 0 && tape[_s+1] == 0 && tape[_s+2] == 0 && tape[_s+3] == 0) break; int _ka = _s + 5; int _ki = _kaddr; int _eq = 1; while (tape[_ka] && tape[_ki]) { if (tape[_ka] != tape[_ki]) { _eq = 0; break; } _ka++; _ki++; } if (_eq && tape[_ka] == tape[_ki]) { bfpp_set(ptr, bfpp_get(_s + 36)); bfpp_set(ptr+4, 1); _found = 1; break; } } if (!_found) { bfpp_set(ptr, 0); bfpp_set(ptr+4, 0); } }\n");
        }
        "__hashmap_set" => {
            // Input: tape[ptr]=map_addr, tape[ptr+4]=key_addr, tape[ptr+8]=value
            indent(out, ctx.indent);
            out.push_str("{ int _maddr = (int)bfpp_get(ptr) & TAPE_MASK; int _kaddr = (int)bfpp_get(ptr+4) & TAPE_MASK; uint64_t _val = bfpp_get(ptr+8); int _cap = (int)bfpp_get(_maddr); uint32_t _h = 5381; for (int _i = _kaddr; tape[_i]; _i++) _h = _h * 33 + tape[_i]; for (int _p = 0; _p < _cap; _p++) { int _s = ((_h + _p) % _cap) * 40 + _maddr + 8; uint32_t _sh = (uint32_t)tape[_s] | ((uint32_t)tape[_s+1]<<8) | ((uint32_t)tape[_s+2]<<16) | ((uint32_t)tape[_s+3]<<24); if (_sh == 0) { tape[_s] = _h & 0xFF; tape[_s+1] = (_h>>8) & 0xFF; tape[_s+2] = (_h>>16) & 0xFF; tape[_s+3] = (_h>>24) & 0xFF; int _ki = _kaddr; int _ko = _s + 5; while (tape[_ki]) tape[_ko++] = tape[_ki++]; tape[_ko] = 0; bfpp_set(_s + 36, _val); bfpp_set(_maddr + 4, bfpp_get(_maddr + 4) + 1); break; } int _ka = _s + 5; int _ki = _kaddr; int _eq = 1; while (tape[_ka] && tape[_ki]) { if (tape[_ka] != tape[_ki]) { _eq = 0; break; } _ka++; _ki++; } if (_eq && tape[_ka] == tape[_ki]) { bfpp_set(_s + 36, _val); break; } } }\n");
        }
